    ...
    do
        ...
        do
            if [ "$version" = "cm_shared" ] || [ "$version" = "cm_shared_filtered" ]; then
                ...
                ./bin/$version.out $universe_size $stream_size $(($buckets*$threads)) $rows 1 1 0 1 $threads $queries 0 >/dev/null
	    	...
	    	echo "Using " $new_columns "columns"
                ./bin/$version.out $universe_size $stream_size $new_columns $rows 1 1 0 1 $threads $queries 0 >/dev/null
            else
		echo "Memory per thread" $(($buckets*$rows*4))
                ./bin/$version.out $universe_size $stream_size $buckets $rows 1 1 0 1 $threads $queries 0 >/dev/null
            fi
