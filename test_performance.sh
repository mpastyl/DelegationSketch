    ...
    do
        ...
        do
            if [ "$version" != "cm_shared" ] && [ "$version" != "cm_shared_filtered" ]; then
                ./bin/$version.out 10000 60000 $buckets $rows 1 1.7 0 1 $threads $queries 1 | grep -oP 'Total processing throughput [+-]?[0-9]+([.][0-9]+)?+' -a --text >> logs/${version}_${queries}_queries.log
            else
                ./bin/$version.out 10000 60000 $buckets $rows 1 1.7 0 1 $threads $queries 1 | grep -oP 'Total processing throughput [+-]?[0-9]+([.][0-9]+)?+' -a --text >> logs/${version}_${queries}_queries.log
            fi
        done
...
do
    ...
    do
        ...
        do
            if [ "$version" != "cm_shared" ] && [ "$version" != "cm_shared_filtered" ]; then
                ./bin/$version.out 10000 60000 $buckets $rows 1 1.7 0 1 $threads $queries 1 | grep -oP 'Total processing throughput [+-]?[0-9]+([.][0-9]+)?+' -a --text >> logs/${version}_${threads}_threads.log
            else
                ./bin/$version.out 10000 60000 $buckets $rows 1 1.7 0 1 $threads $queries 1 | grep -oP 'Total processing throughput [+-]?[0-9]+([.][0-9]+)?+' -a --text >> logs/${version}_${threads}_threads.log
            fi
        done
