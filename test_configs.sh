...
do
    ...
    do
        ./$version.out 10000 60000 64 64 1 1 0 1 $threads 0 1 | grep -oP 'Total processing throughput [+-]?[0-9]+([.][0-9]+)?+' -a --text >> ${version}_no_queries.log
    done
done
...
do
    ...
    do
        ...
        for queries in $query_rates
        do
            ./$version.out 10000 60000 64 64 1 1 0 1 $threads $queries 1 | grep -oP 'Total processing throughput [+-]?[0-9]+([.][0-9]+)?+' -a --text >> ${version}_${threads}_threads.log
        done
    done
done