...
do
    ...
    do
        rm -f logs/${version}_${queries}_queries_1_5_skew_10_times_final_no_squash.log
        for threads in $thread_list
        do
	for rep in `seq 1 10`
	do
            if [ "$version" = "cm_shared" ] || [ "$version" = "cm_shared_filtered" ]; then
                ./bin/$version.out $universe_size $stream_size $(($buckets*$rows)) $rows 1 1.5 0 1 $threads $queries 1 | grep -oP 'Total processing throughput [+-]?[0-9]+([.][0-9]+)?+' -a --text >> logs/${version}_${queries}_queries_1_5_skew_10_times_final_no_squash.log
                ...
                ./bin/$version.out $universe_size $stream_size $new_columns $rows 1 1.5 0 1 $threads $queries 1 | grep -oP 'Total processing throughput [+-]?[0-9]+([.][0-9]+)?+' -a --text >> logs/${version}_${queries}_queries_1_5_skew_10_times_final_no_squash.log
            else
                ./bin/$version.out $universe_size $stream_size $buckets $rows 1 1.5 0 1 $threads $queries 1 | grep -oP 'Total processing throughput [+-]?[0-9]+([.][0-9]+)?+' -a --text >> logs/${version}_${queries}_queries_1_5_skew_10_times_final_no_squash.log
            fi
        done
	done
    done
...
do
    ...
    do
        echo ${version} ${threads}
        rm -f logs/skew_${version}_${threads}_threads_${queries}_queries_final_no_squash.log
        for skew in $skew_rates
	do
            if [ "$version" = "cm_shared" ] || [ "$version" = "cm_shared_filtered" ]; then
                ./bin/$version.out $universe_size $stream_size $(($buckets*$rows)) $rows 1 $skew 0 1 $threads $queries 1 | grep -oP 'Total processing throughput [+-]?[0-9]+([.][0-9]+)?+' -a --text >> logs/skew_${version}_${threads}_threads_${queries}_queries_final_no_squash.log
            elif [ "$version" = "cm_delegation_filters" ] || [ "$version" = "cm_delegation_filters_with_linked_list" ]; then
                new_columns=$((($buckets*$rows*4 - $threads*64)/($rows*4)))
                ./bin/$version.out $universe_size $stream_size $new_columns $rows 1 $skew 0 1 $threads $queries 1 | grep -oP 'Total processing throughput [+-]?[0-9]+([.][0-9]+)?+' -a --text >> logs/skew_${version}_${threads}_threads_${queries}_queries_final_no_squash.log
            else
                ./bin/$version.out $universe_size $stream_size $buckets $rows 1 $skew 0 1 $threads $queries 1 | grep -oP 'Total processing throughput [+-]?[0-9]+([.][0-9]+)?+' -a --text >> logs/skew_${version}_${threads}_threads_${queries}_queries_final_no_squash.log
            fi
	done
    done
    done
